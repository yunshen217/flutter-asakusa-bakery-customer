...
class _AddToCartAnimationState extends State<AddToCartAnimation> {
                            ...
                            ? TweenAnimationBuilder(
                                ...
                                builder: (context, double value, widget) {
                                  return Transform.rotate(
                                      angle: value,
                                      child: Opacity(opacity: model.opacity, child: widget));
                                })
                            : Opacity(opacity: model.opacity, child: model.container))
                    : Container())
                .toList(),
          ),
        ),
  ...
  Future<void> runAddToCartAnimation(GlobalKey widgetKey) async {
    ...
    animationModel.imageSourcePoint =
        Offset(widgetKey.globalPaintBounds!.top - 100, widgetKey.globalPaintBounds!.left + 20);
    ...
    animationModel.imageDestPoint = Offset(
        widgetKey.globalPaintBounds!.top - (startingHeight + widget.height - 20),
        widgetKey.globalPaintBounds!.left + 20);

    animationModel.imageSourceSize =
        Size(widgetKey.currentContext!.size!.width, widgetKey.currentContext!.size!.height);

    animationModel.imageDestSize = Size(widgetKey.currentContext!.size!.width + widget.width - 70,
        widgetKey.currentContext!.size!.height + widget.height - 70);
    ...
    animationModel.imageDestPoint = Offset(
        widget.cartKey.globalPaintBounds!.top - 100, widget.cartKey.globalPaintBounds!.left + 10);

    animationModel.imageDestSize = Size(widget.cartKey.currentContext!.size!.width - 100,
        widget.cartKey.currentContext!.size!.height - 40);
